...
- **Offline-First:** All state is persisted locally using Zustand persist.
...
- **Components:** Shadcn UI (Radix primitives), Vaul (for mobile bottom drawers).
- **Icons:** Lucide React.
...
The core application is fully functional. The state is managed locally and persists across reloads. The UI is polished with responsive layouts, and the underlying mathematical algorithms (balances, custom splitting logic, settlement) are fully implemented and active. Deployment configuration for GitHub Pages (`gh-pages`) is also set up.
